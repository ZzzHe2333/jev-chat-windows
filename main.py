...
def fill_reply(text, *, auto=False, expected_chat=None):
    ...
    if auto:
        # 自动发送必须同时满足三件事：开关+白名单、OCR 当前会话仍没变、界面正在看的也是该会话。
        # 前台窗口检查放在 app.fill.fill() 里，并且粘贴后按 Enter 前还会再检查一次。
    ...
    if settings.reply_target() and ov.at_prefix_enabled():
        target = target_of(current)  # 填进去的是界面上正看着的那个会话的对象
        if target:
                    ...
                    if settings.auto_send_allowed(title):
                        ...
                        if cands:
                            try:
                                ...
                                # 失败不做任何“补发”动作，候选仍留在界面上，用户可以自己确认后手动填入。
