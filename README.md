...
一键填入微信输入框。**默认仍由你手动发送；也可以在设置里开启“自动发送（白名单）”。自动发送只对精确匹配的白名单会话生效，而且生成完成时微信主窗口必须仍是系统当前前台窗口。**
...
- 可选自动发送：设置里开启后，只有白名单里的当前会话才会自动发“推荐回复”；生成结束时微信如果已经不是系统前台窗口，就只保留候选、不发送。
...
- **3 条候选**：每条带 Jev 给的胜出概率百分比，按概率排序，推荐那条置顶并标「推荐回复」；
  每条都有「填入微信」和复制按钮。
- **自动发送（可选）**：默认关闭；只能对白名单中精确匹配的当前会话自动发送推荐回复，并要求微信主窗口保持前台；任何检查失败都不发送。
...
- **自动发送默认关闭，并有两道硬限制。** 只有设置中的白名单会话可以自动发送，并且发送瞬间微信主窗口必须仍是系统当前前台窗口；会话切换、窗口切走、白名单不匹配都会取消自动发送，只留下候选供你手动处理。
...
| 自动发送（白名单） | 默认关；开启时必须至少配置一个白名单会话，且只自动发送推荐回复 | `config.json` → `auto_send`（默认关） |
| 自动发送白名单 | 每行一个会话名，和 OCR 识别到的当前会话标题精确匹配 | `config.json` → `auto_send_whitelist` |
...
- **自动发送（可选）**：设置里开启并配置白名单后，白名单中的当前会话会自动发送推荐回复；发送前会重新确认 OCR 当前会话没有变化，而且微信主窗口仍是系统前台窗口。
...
app/                    UI + 采集层
  ...
  fill.py               默认填入不发送；白名单自动发送路径额外校验当前会话与前台窗口后才按 Enter
