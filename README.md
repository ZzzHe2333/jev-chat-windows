...
一键填入微信输入框。**默认仍由你手动发送；也可以在设置里开启“自动发送（白名单）”。自动发送只对白名单精确匹配的会话生效；微信在后台时可短暂激活微信切换/发送，完成后尽量恢复原窗口。**
...
- 默认：点「填入微信」→ 文字进微信输入框 → **你自己看一眼、改一改、按发送**。
- 可选自动发送：设置里开启后，当前白名单会话可自动发“推荐回复”；微信在后台时，还会巡检左侧带未读红点、且侧栏标题精确命中白名单的会话，短暂切过去处理后尽量恢复原前台窗口。
...
- **3 条候选**：每条带 Jev 给的胜出概率百分比，按概率排序，推荐那条置顶并标「推荐回复」；
  每条都有「填入微信」和复制按钮。
- **自动发送（可选）**：默认关闭；只对白名单精确匹配的会话发送推荐回复。后台时会巡检左侧未读白名单，会话切换后还会用头部 OCR 二次核对；切换/发送失败都不发送。
...
- **自动发送默认关闭，并坚持白名单双重核对。** 后台巡检只会点击“未读红点所在行 + 侧栏 OCR 标题精确命中白名单”的会话；切换后还必须由聊天头部 OCR 再次读到同一个标题。真正发送时会短暂激活微信，按 Enter 前目标微信仍必须保持焦点；任一步失败都取消，不补发。
...
| 自动发送（白名单） | 默认关；开启后当前白名单会话可自动发推荐回复，微信在后台时还会巡检侧栏未读白名单 | `config.json` → `auto_send`（默认关） |
| 自动发送白名单 | 每行一个会话名；侧栏巡检先精确匹配一次，切换后再用头部 OCR 精确核对 | `config.json` → `auto_send_whitelist` |
...
- **采集开关**：标题栏右上角。拨到「已暂停」就完全不读微信（WGC 会话一起停掉，黄框也没了），
  已经生成的候选照样能填入、能复制。
- **填入微信**：点候选卡片上的「填入微信」，文字进微信输入框，光标留在那儿，默认仍由你自己发送。
- **自动发送（可选）**：设置里开启并配置白名单后，当前白名单会话会自动发送推荐回复；微信在后台时会巡检左侧未读白名单，短暂切换并二次核对标题后发送。你正在微信前台操作时，程序不会主动切会话。
...
app/                    UI + 采集层
  ...
  fill.py               默认填入不发送；自动发送可短暂激活微信，按 Enter 前仍校验目标窗口，结束后尽量恢复原前台
...
**未发版**
- 后台自动发送：微信不在前台时可短暂激活微信完成发送，结束后尽量恢复原前台窗口
- 白名单多会话巡检：监听左侧未读红点，只点击侧栏 OCR 标题精确命中白名单的会话；切换后用头部 OCR 二次核对
- 微信正在前台由用户操作时不主动切会话；切换失败、标题不一致、焦点丢失均取消发送且不补发

**v0.1.11**
- 正式版基于上游 v0.1.10，同步保留上游新功能并加入下游自动化增强
- 下游增强：新增白名单自动发送，默认关闭；只对 OCR 当前会话标题精确匹配的白名单生效
- 自动发送不抢焦点：微信必须保持为 Windows 当前前台窗口，粘贴后按 Enter 前再次校验；失败不补发
